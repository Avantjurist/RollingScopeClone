...
function timeSpanToString(startDate, endDate) {
    ...
    str += res.toTimeString().slice(0, 8);
    var milliSeconds = (endDate.getMilliseconds() - startDate.getMilliseconds()).toString();
...
function angleBetweenClockHands(date) {
    ...
    var angel = Math.abs(0.5 * (60 * hours + date.getUTCMinutes()) - 6 * date.getUTCMinutes()) * (Math.PI / 180);

    if ((angel / Math.PI) % 2 == 1) {
        return Math.PI;
    } else {
        return angel % Math.PI;
    }
}
