 ...
 */
function getPolynom() {
    var arg = Array.from(arguments).reverse();
    return function(x) {
        return arg
            .reduceRight((pv, cv, i) => {
                return pv += cv * Math.pow(x, i);
            }, 0)
    }
}
...
function memoize(func) {
    var storage = {};
    return function() {
        var keyMemoize = JSON.stringify(arguments);
        if (keyMemoize in storage) {
            return storage[keyMemoize];
        } else {
            return storage[keyMemoize] = func.apply(this, arguments);
        }
    }
}
...
function logger(func, logFunc) {
    return function() {
        var result;
        var args = [].reduce.call(arguments, function(result, current) {
            return result == '' ? result += JSON.stringify(current) : result += ',' + JSON.stringify(current);
        }, '');
